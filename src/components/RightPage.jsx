    ...
    <div style={{ width: "90%", display: "flex", flexDirection: "column", alignItems: "center", paddingLeft: 20, paddingTop: 30 }} >
      {item.subquests.map((subQ, index) =>
        <React.Fragment>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", justifyContent: "flex-start", width: "95%" }}>
            {subQ.notes.split("\n").map(item => {
              if (item.length > 0) return <p style={{ fontSize: 15, width: "95%", textAlign: "left" }} onClick={() => { setSubquestEditMode(true); setOpen(true); setSubquestEditIndex(index); }}>{item}</p>
            })}
          </div>
        </React.Fragment>
      )}
